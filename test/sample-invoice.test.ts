...
import { generateXRechnungPdf } from '../src/generate.js';
...
describe('generateXRechnungPdf', () => {
  it('returns a ZUGFeRD/XRechnung PDF for the sample invoice fixture', async () => {
    const signatureImage = await readFile(new URL('./fixtures/signature.png', import.meta.url));
    const pdf = await generateXRechnungPdf(sampleInvoice, {
      lang: 'en-us',
      signatureImage,
    });
